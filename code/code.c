...
int total[SWN] = {0}; // 修复：声明并初始化total数组
...
int main() {
    ...
    struct node *head;
    int total[SWN] = {0}; // 修复：声明并初始化total数组
    printf("Please input the students' marks record file name: ");
    scanf("%s", stuf);

    if ((fp = fopen(stuf, "r")) == NULL) {
        printf("The file %s doesn't exist. Do you want to create it? (Y/N) ", stuf);
        getchar();
        c = getchar();
        if (c == 'Y' || c == 'y') {
            fp = fopen(stuf, "w");
            printf("Please input the number of records you want to write to the file: ");
            scanf("%d", &n);
            for (i = 0; i < n; i++) {
...
int retrievebyn(char *fname, char *key) {
    ...
    if (c == 0) {
        printf("The student's name %s doesn't exist.\n", key);
    }
...
int retrievebyc(char *fname, char *key) {
    ...
    if (c == 0) {
        printf("The student's code %s doesn't exist.\n", key);
    }
